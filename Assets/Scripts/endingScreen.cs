...
public class endingScreen : MonoBehaviour
{
	...
	void Awake()
	{
		...
		StartCoroutine(GetRequest(HOST + "/api/beats/getlb"
			+ "?mode=" + PlayerPrefs.GetInt("songchoice").ToString(), "GetLBInfo"));
		if (PlayerPrefs.GetFloat("myScore") > PlayerPrefs.GetFloat("myHighscore"))
		{
			// new high score
			PlayerPrefs.SetFloat("myHighscore", PlayerPrefs.GetFloat("myScore"));
			StartCoroutine(GetRequest(HOST + "/api/beats/getrank?myScore="
				+ PlayerPrefs.GetFloat("myHighscore").ToString("0")
	...
	void Start()
	{
		...
		DispScore.text = PlayerPrefs.GetFloat("GameScore").ToString("0");
	}
